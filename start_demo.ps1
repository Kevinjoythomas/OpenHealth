...
$Ngrok  = "ngrok"   # assumes ngrok is on PATH; adjust if needed
...
function Start-Svc($name, $port) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$svcPath'; $cmd" `
        -WindowStyle Normal
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "& '$Ngrok' start backend" `
    -WindowStyle Normal
...
try {
    ...
    Write-Host "  !!  Ollama not running — start it from system tray first" -ForegroundColor Red
}
...
Write-Host ""
Write-Host "  Vercel (public):  https://open-health.vercel.app" -ForegroundColor White
